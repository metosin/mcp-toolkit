...
(defn request-set-logging-level [context level]
  (json-rpc.handler/call-remote-method context {:method "logging/setLevel"
                                                :params {:level level}}))

(defn request-complete-prompt-param [context prompt-name
                                     param-name param-value]
  (json-rpc.handler/call-remote-method context {:method "completion/complete"
...
(defn request-prompt-list [context]
  (let [{:keys [session]} context
    ...
    (when (contains? server-capabilities :prompts)
      (-> (json-rpc.handler/call-remote-method context {:method "prompts/list"})
...
(defn request-prompt [context prompt-name arguments]
  (json-rpc.handler/call-remote-method context {:method "prompts/get"
...
(defn request-resource-list [context]
  (let [{:keys [session]} context
    ...
    (when (contains? server-capabilities :resources)
      (-> (json-rpc.handler/call-remote-method context {:method "resources/list"})
...
(defn request-resource [context resource-uri]
  (json-rpc.handler/call-remote-method context {:method "resources/read"
                                                :params {:uri resource-uri}}))

(defn request-resource-template-list [context]
  (json-rpc.handler/call-remote-method context {:method "resources/templates/list"}))


(defn request-subscribe-resource [context resource-uri]
  (json-rpc.handler/call-remote-method context {:method "resources/subscribe"
                                                :params {:uri resource-uri}}))

(defn request-unsubscribe-resource [context resource-uri]
  (json-rpc.handler/call-remote-method context {:method "resources/unsubscribe"
                                                :params {:uri resource-uri}}))
...
(defn request-tool-list [context]
  (let [{:keys [session]} context
    ...
    (when (contains? server-capabilities :prompts)
      (-> (json-rpc.handler/call-remote-method context {:method "tools/list"})
...
(defn request-tool-invocation [context tool-name arguments]
  (json-rpc.handler/call-remote-method context {:method "tools/call"
                                                :params {:name tool-name
                                                         :arguments arguments}}))

(defn notify-cancel-request [context request-id]
  (json-rpc.handler/send-message context (json-rpc.message/notification "cancelled"
                                                                        {:requestId request-id})))

(defn notify-root-list-updated [context]
  (json-rpc.handler/send-message context (json-rpc.message/notification "roots/list_changed")))

(defn send-first-handshake-message [context]
  (let [{:keys [session]} context
        {:keys [client-info
                protocol-version]} @session]
    (-> (json-rpc.handler/call-remote-method context {:method "initialize"
